...
\author{\pdftooltip{Rafid Ahmed}{Student ID: 210041232}}
\affiliation{
...
\email{rafidahmed816@gmail.com}

\author{\pdftooltip{Muntasir Hossain}{Student ID: 210041265}}
\affiliation{
  \institution{Department of Computer Science and Engineering\\
  Islamic University of Technology (IUT), OIC}
  \city{Dhaka}
...
\author{\pdftooltip{Ifti Akkas}{Student ID: 210041111}}
\affiliation{
  \institution{Department of Computer Science and Engineering\\
  Islamic University of Technology (IUT), OIC}
  \city{Dhaka}
...
\email{iftiakkas@iut-dhaka.edu}

\author{\pdftooltip{Rubaeeyat Hossain}{Student ID: 210041266}}
\affiliation{
  \institution{Department of Computer Science and Engineering\\
  Islamic University of Technology (IUT), OIC}
  \city{Dhaka}
...
\email{rubaeeyat@iut-dhaka.edu}

\author{\pdftooltip{Zawad Bin Shoukat}{Student ID: 210041257}}
\affiliation{
  \institution{Department of Computer Science and Engineering\\
  Islamic University of Technology (IUT), OIC}
  \city{Dhaka}
...
\email{zawadtasinnn@gmail.com}

\author{\pdftooltip{Syed Huzzatullah Mihad}{Student ID: 210041218}}
\affiliation{
  \institution{Department of Computer Science and Engineering\\
  Islamic University of Technology (IUT), OIC}
  \city{Dhaka}
...
\email{huzzatullah@iut-dhaka.edu}
...
Figure~\ref{fig:arch} shows how the stages are distributed across the two
sides of the system. The device is responsible only for selecting the
application and rendering the result; extraction, classification,
attribution and explanation all run server-side, where the model and the
language layer can be updated without shipping a new client.
...
\begin{tikzpicture}[
  ...
  box/.style   = {draw, rounded corners=2pt, align=center,
                  text width=0.62\columnwidth, inner sep=5pt},
  stage/.style = {draw, fill=black!4, rounded corners=1.5pt, align=center,
                  text width=0.46\columnwidth, inner sep=3.2pt,
                  font=\footnotesize},
...
% --- client: selection ---
\node[box] (app) {\textbf{Android Application}\\[1pt]
  \footnotesize User selects an application};

% --- server pipeline ---
\node[stage, below=13mm of app] (extract) {APK feature extraction};
\node[stage, below=of extract]  (clf)     {LightGBM risk classifier};
\node[stage, below=of clf]      (shap)    {SHAP attribution};
\node[stage, below=of shap]     (llm)     {Qwen LLM explanation layer};

\draw[flow] (extract) -- (clf);
\draw[flow] (clf) -- (shap);
\draw[flow] (shap) -- (llm);

\begin{scope}[on background layer]
  \node[draw, rounded corners=2pt, fill=black!1,
        fit=(extract)(clf)(shap)(llm),
        inner sep=7pt, inner ysep=13pt] (server) {};
\end{scope}
\node[anchor=north, font=\small\bfseries, yshift=-2.5pt]
  at (server.north) {Analysis Server};

% --- client: report ---
\node[box, below=13mm of server] (ui) {\textbf{Android Interface}\\[1pt]
  \footnotesize Low / Medium / High risk,\\[-1pt]
  \footnotesize score and explanation};

% --- cross-boundary flows ---
\draw[flow] (app) -- node[lbl, right] {APK / application information}
  (server.north);
\draw[flow] (server.south) -- node[lbl, right] {risk score + explanation}
  (ui);

\end{tikzpicture}
\caption{System architecture. The device selects the application and
displays the result; feature extraction, risk classification, SHAP
attribution and explanation generation run on the analysis server. Each
stage consumes only the output of the previous one, so every sentence the
user reads is traceable back to an attributed feature.}
\label{fig:arch}
\end{figure}
...
The explanation layer is a Qwen large language model, prompted with the
risk band and the ranked SHAP attributions and instructed to restate them
within a fixed length. It receives no other information about the
application, which is what makes the traceability requirement (NFR4)
enforceable rather than aspirational: the model has nothing to draw on
except what the classifier and the attribution produced. The disclaimer
text is held outside the generated region so that it cannot be rephrased
or softened.
